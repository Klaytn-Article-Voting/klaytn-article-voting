...
const SignInModal = ({
  ...
  useEffect(() => {
    if (isError && errorConnect && !isLoadingSwitchNetwork) {
      setSignInClicked(false)
      setError(
        isErrorSwitchNetwork && errorSwitchNetwork
          ? errorSwitchNetwork.message
          : errorConnect.message,
      )
    }
  }, [isError, errorConnect, isLoadingSwitchNetwork, isErrorSwitchNetwork, errorSwitchNetwork])

  useEffect(() => {
    console.log(address, chain, signInClicked)
    if (address && chain && signInClicked) {
      if (chain.id === CONFIGS.CHAIN_ID) {
        toast.success('Connect wallet successfully. Processing login...')
        console.log('success')
        setError('')
  ...
  return (
    <Modal showModal={showSignInModal} setShowModal={setShowSignInModal}>
      <div className="w-full max-w-sm rounded-lg bg-white p-4 dark:border-gray-700 dark:bg-gray-800 sm:p-6 md:border md:border-gray-200 md:shadow">
        ...
        <ul className="my-4 space-y-3">
          ...
          <li>
            ...
            >
              ...
              <span className="ml-3 flex flex-1 whitespace-nowrap text-left">
                <span className="mr-1">MetaMask</span>
